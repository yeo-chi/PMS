...
// CancelConfirmRequestKey/ChangeRequestKey와 대칭되는 형식: 호출자가 externalRequestId를 주면
// (예: 호스트 어드민 UI가 요청 단위로 발급하는 값) 그 값으로 정확히 식별하고, 없으면 초 단위로
// 묶어 "같은 초 안에 도착한 동일 재시도"만 이 키로 멱등 처리한다. externalRequestId 없이 같은 초
// 안에서 진짜로 서로 다른 두 요청이 들어오면(흔치 않지만) 하나로 합쳐질 수 있는데, 이 경우에도
// CancelRequest의 상태 기반 멱등성(PENDING_CANCEL/CANCELLED면 재전이 없이 감사 기록만 추가)이
// 최종 방어선이 되어 안전하다.
