...
class CancelConfirmRequestKeyTest : FeatureSpec({
    feature("CancelConfirmCommand.buildCancelConfirmRequestKey") {
        ...
        scenario("같은 입력이면 항상 같은 키를 생성한다") {
            val first = command("OTA_AGODA", "REF-2", "EXT-2").buildCancelConfirmRequestKey(now)
            val second = command("OTA_AGODA", "REF-2", "EXT-2").buildCancelConfirmRequestKey(now)

            first shouldBe second
        }

        scenario("externalRequestId가 다르면 다른 키를 생성한다") {
            val first = command("OTA_AGODA", "REF-3", "EXT-A").buildCancelConfirmRequestKey(now)
            val second = command("OTA_AGODA", "REF-3", "EXT-B").buildCancelConfirmRequestKey(now)
