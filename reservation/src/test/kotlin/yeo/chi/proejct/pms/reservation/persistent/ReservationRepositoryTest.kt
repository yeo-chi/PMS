...
class ReservationRepositoryTest : FeatureSpec({
    ...
    feature("ReservationRepository") {
        scenario("save 호출 시 id가 채번된 엔티티가 반환된다") {
        ...
        scenario("findByPlatformCodeAndCode로 조회되면 도메인으로 변환 가능한 엔티티를 반환한다") {
            val reservationRepository = mockk<ReservationRepository>()
        ...
        scenario("findByPlatformCodeAndCode로 조회되지 않으면 null을 반환한다 (존재 여부는 null 체크로 판단)") {
            val reservationRepository = mockk<ReservationRepository>()
