...
class ReservationRepositoryTest(
        ...
        scenario("저장 후 reservation_no가 platform_id:platform_reservation_ref 형태로 채워진다") {
            val saved =
                reservationRepository.saveAndFlush(
                    newReservation("REF-1", "ROOM-101", LocalDate.of(2026, 1, 1), LocalDate.of(2026, 1, 5)),
                )

            saved.reservationCode shouldBe "OTA_BOOKING:REF-1"
        }

        scenario("신규 저장 시 version은 1로 저장된다") {
            val saved =
                reservationRepository.saveAndFlush(
                    newReservation("REF-2", "ROOM-102", LocalDate.of(2026, 2, 1), LocalDate.of(2026, 2, 5)),
                )
    ...
    feature("같은 room_code에 대한 날짜 겹침 방지 (excl_room_date_overlap)") {
        scenario("CONFIRMED 상태끼리 room_code가 같고 날짜가 겹치면 저장이 거부된다") {
            reservationRepository.saveAndFlush(
                newReservation("REF-3", "ROOM-201", LocalDate.of(2026, 3, 1), LocalDate.of(2026, 3, 10)),
            )

            shouldThrow<DataIntegrityViolationException> {
                reservationRepository.saveAndFlush(
                    newReservation("REF-4", "ROOM-201", LocalDate.of(2026, 3, 5), LocalDate.of(2026, 3, 15)),
                )
            }
        }
        ...
        scenario("CONFIRMED와 PENDING_CANCEL도 겹치면 저장이 거부된다") {
            reservationRepository.saveAndFlush(
                newReservation("REF-5", "ROOM-202", LocalDate.of(2026, 4, 1), LocalDate.of(2026, 4, 10)),
            )
        ...
        scenario("날짜가 겹치지 않으면 같은 room_code라도 저장에 성공한다") {
            reservationRepository.saveAndFlush(
                newReservation("REF-7", "ROOM-203", LocalDate.of(2026, 5, 1), LocalDate.of(2026, 5, 5)),
            )

            val saved =
                reservationRepository.saveAndFlush(
                    newReservation("REF-8", "ROOM-203", LocalDate.of(2026, 5, 5), LocalDate.of(2026, 5, 10)),
                )
    ...
    feature("reservation_no 유일성 (uq_reservation_no)") {
        scenario("동일한 platform_id + platform_reservation_ref로 재삽입하면 저장이 거부된다") {
            reservationRepository.saveAndFlush(
                newReservation("REF-DUP", "ROOM-301", LocalDate.of(2026, 7, 1), LocalDate.of(2026, 7, 5)),
            )

            shouldThrow<DataIntegrityViolationException> {
                reservationRepository.saveAndFlush(
                    newReservation("REF-DUP", "ROOM-302", LocalDate.of(2026, 8, 1), LocalDate.of(2026, 8, 5)),
                )
            }
        }
    }
    ...
    feature("낙관적 락 (version)") {
        scenario("로드 이후 다른 트랜잭션이 먼저 커밋하면, 로드 시점의 stale version으로 시도한 업데이트는 실패한다") {
            val saved =
                reservationRepository.saveAndFlush(
                    newReservation("REF-LOCK", "ROOM-401", LocalDate.of(2026, 9, 1), LocalDate.of(2026, 9, 5)),
                )
