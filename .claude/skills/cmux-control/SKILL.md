...
This skill enables agents to interact with and control the CMUX environment using its Unix socket API command-line
interface (`cmux`). CMUX is an advanced terminal multiplexer featuring customizable sidebars (status, progress,
logging), workspace/pane layouts, and a built-in interactive browser automation engine.
...
5. Upon successful completion, set status to green (e.g.
   `cmux set-status "Mode" "Idle" --icon "check" --color "#34C759"`) and clear the progress bar: `cmux clear-progress`.
